...
public class HelloWorld {
	 ...
	 * 
	 * //枚举遍历 for(Season s:Season.values()){ System.out.println(s); }
	 * 
	 * //对二维数组进行重新排序，转变为字符串再进行的排序。 int[][] a = new int[5][8]; for(int i=0;
	 * i<a.length; i++){ for(int j=0; j<a[i].length; j++){
	 * a[i][j]=(int)(Math.random()*100); //System.out.print(a[i][j]+" "); }
	 * //System.out.println(); } //System.out.println("=====================");
	 * for (int[] i : a) { System.out.println(Arrays.toString(i)); }
	 * System.out.println("***********************************************");
	 * int[] b = new int[a.length*a[0].length]; for(int i=0; i<a.length; i++){
	 * System.arraycopy(a[i], 0, b, i*a[i].length, a[i].length); }
	 * Arrays.sort(b); for(int i=0; i<a.length; i++){ System.arraycopy(b,
	 * i*a[i].length, a[i], 0, a[i].length); } for(int[] i:a){
	 ...
	 * 
	 * int[] a = new int[]{18, 62, 68, 82, 65, 9}; int[] c = new int[]{18, 62,
	 * 68, 82, 65, 8}; System.out.println(Arrays.equals(a, c));
	...
	/*
	 * //二位数组的阵列 int[][] a = new int[5][5]; for(int i=0; i<a.length; i++){
	 * for(int j=0; j<a[i].length; j++){ a[i][j]=(int)(Math.random()*100); } }
	 * for(int[] row:a){ for(int each:row){ System.out.print(each+" "); }
	 ...
	 * ); } System.out.println(); int[] b = new int[(int)(Math.random()*5+5)];
	 ...
	 * a.length, b.length); System.out.println("数组c的值是a和b的值之和："); for(int i:c){
	 * System.out.print(i+" "); }
	 */

	/*
	 * int a [] = new int[]{18,62,68,82,65,9}; int b[] = new
	 * int[3];//分配了长度是3的空间，但是没有赋值 //通过数组赋值把，a数组的前3位赋值到b数组 //方法一： for循环 for (int
	 * i = 0; i < b.length; i++) { b[i] = a[i]; } //方法二: System.arraycopy(src,
	 * srcPos, dest, destPos, length) //src: 源数组 //srcPos: 从源数组复制数据的启始位置 //dest:
	 * 目标数组 //destPos: 复制到目标数组的启始位置 //length: 复制的长度 System.arraycopy(a, 0, b, 0,
	 * 3); //把内容打印出来 for (int i = 0; i < b.length; i++) { System.out.print(b[i]
	 * + " "); }
	 */

	/*
	 * int values [] = new int[]{18,62,68,82,65,9}; //数组中的内容是 for (int each :
	 * values) { System.out.print(each+" "); } System.out.println(); int max =
	 * -1; for (int each : values) { if(each>max) max = each; }
	 ...
	 * { for (int i = j+1; i < a.length; i++) { if(a[i]<a[j]){ int temp = a[j];
	 * a[j] = a[i];//主要就是这一步！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！ a[i] =
	 * temp; } } } for (int i = 0; i < a.length; i++) { System.out.print(a[i] +
	 * " "); } System.out.println();
	 * System.out.println("========================");
	 * 
	 * //冒泡排序法，虽然都需要i,j,但是比较的是a[i]与a[i+1]的大小，i与j之间没有关系，i,j都是从0开始。
	 * System.out.println("冒泡排序--从大到小排序"); for (int j = 0; j < a.length; j++) {
	 * for (int i = 0; i < a.length-1-j; i++) { if(a[i]<a[i+1]){ int temp =
	 * a[i]; a[i] = a[i+1];//主要就是这一步！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
	 * a[i+1] = temp; } } }
	 * 
	 ...
	 * System.out.println(a[i]); } System.out.println("----------------"); int
	 * min = 100; for(int i=0; i<a.length; i++){ if(a[i]<min){ min=a[i]; } }
 ...
 * // 枚举遍历 for (Season s : Season.values()) { System.out.println(s); }
 * 
 * // 对二维数组进行重新排序，转变为字符串再进行的排序。 int[][] a = new int[5][8]; for (int i = 0; i <
 * a.length; i++) { for (int j = 0; j < a[i].length; j++) { a[i][j] = (int)
 * (Math.random() * 100); // System.out.print(a[i][j]+" "); } //
 * System.out.println(); } // System.out.println("====================="); for
 * (int[] i : a) { System.out.println(Arrays.toString(i)); }
 * System.out.println("***********************************************"); int[]
 * b = new int[a.length * a[0].length]; for (int i = 0; i < a.length; i++) {
 * System.arraycopy(a[i], 0, b, i * a[i].length, a[i].length); } Arrays.sort(b);
 * for (int i = 0; i < a.length; i++) { System.arraycopy(b, i * a[i].length,
 * a[i], 0, a[i].length); } for (int[] i : a) {
 * System.out.println(Arrays.toString(i)); }
 */
